...
<body>
	...
	<h3>숫자 패턴</h3>
	<c:set var="pi" value="3.14159265359" />
	<h4><fmt:formatNumber value="${pi }"/></h4>
	<h4><fmt:formatNumber value="${pi }" pattern="#.##" /></h4>
	<h4><fmt:formatNumber value="${pi }" pattern="0.00" /></h4>
	<h4><fmt:formatNumber value="${pi }" pattern="0.000000000000" /></h4> <!-- 원 데이터 이상의 범위를 넘어가면 0으로 표시처리함 -->
	<h4><fmt:formatNumber value="${pi }" pattern="#.############" /></h4> <!-- 원 데이터 이상의 범위를 넘어가면 더 이상 표기x -->
	...
	<h4>${now }</h4>
	<fmt:formatDate value="${now }" pattern="yyyy년 MM년 dd일 HH시 mm분 ss초" />
